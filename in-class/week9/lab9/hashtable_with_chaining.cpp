...
struct HashTable{
	...
	//add your code below
        ...
        LinkedList *li = &password[hash];
        Node *currentNode = li->head;
        while (currentNode != NULL) {
            if (currentNode->user_name == user_name) {
                cout << "User password for '" << user_name << "': " << currentNode->password << endl;
                return;
            }
            currentNode = currentNode->link;
        }
        cout << "User '" << user_name << "' not found in the hashtable." << endl;
    }
};
